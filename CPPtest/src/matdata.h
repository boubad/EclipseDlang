	...
	template <typename T = int,
		...
		public:
			...
			MatData(const size_t r, const size_t c, const CONTAINER1 &oData,
				...
				const CONTAINER2 *pColNames = nullptr) :
				_rows(r), _cols(c) {
				assert(this->_rows > 0);
				assert(this->_cols > 0);
			...
			MatData(const MatDataType &other) :BaseInfo(other), _rows(other._rows), _cols(other._cols), _data(other._data),
				_rowids(other._rowids), _colids(other._colids) {}// MatData
		...
		public:
			...
			virtual OStreamType & write_to(OStreamType &os) const {
				...
				info_write_json_value(os, STRING_COL_NAMES, this->colNames()) << STRING_COMMA << std::endl;
		...
		public:
			...
			template <typename X, class ALLOCX, class ALLOCSS>
			bool to_recoded(MatData<X, ALLOCX, ALLOCSS> &oRes, const X xmax = 1000, const X xmin = 0) const {
				if (xmax <= xmin) {
					return (false);
				}
				if (!this->is_valid()) {
