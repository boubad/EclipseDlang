...
namespace info {
	...
	protected:
		...
		DistanceType _type;
	public:
		DistanceFunc() :_valid(true), _type(DistanceType::distanceSquare) {}
		DistanceFunc(const DistanceFuncType &other) :BaseInfo(other), _valid(other._valid), _type(other._type) {}
	...
	protected:
		...
		template <class CONTAINER1, class CONTAINER2>
		double perform_compute(const CONTAINER1 & v1,
			const CONTAINER2 & v2,
			const size_t irow1, const size_t irow2) const {
			assert(this->is_valid());
			...
			for (size_t i = 0; i < n; ++i) {
				res += perform_one_step((double)v1[i], (double)v2[i], i, irow1, irow2);
			}// i
	...
	public:
		typedef DistanceFunc<Z> DistanceFuncType;
		typedef ManhattanDistanceFunc<Z> ManhattanDistanceFuncType;
	public:
	...
	public:
		typedef DistanceFunc<Z> DistanceFuncType;
		typedef MaxDistanceFunc<Z> MaxDistanceFuncType;
	public:
		MaxDistanceFunc() {
			this->_type = DistanceType::distanceMax;
		}
		MaxDistanceFunc(const MaxDistanceFuncType &other) :DistanceFuncType(other) {
		}
	...
	protected:
		...
		double perform_compute(const CONTAINER1 & v1,
			...
			double res = 0;
			for (size_t i = 0; i < n; ++i) {
				double d = perform_one_step((double)v1[i], (double)v2[i], i, irow1, irow2);
				if (i == 0) {
					res = d;
				}
				else if (d > res) {
	...
	//////////////////////////////////////////
	template <typename Z = long> class SquareDistanceFunc : public DistanceFunc<Z> {
	public:
		typedef DistanceFunc<Z> DistanceFuncType;
		typedef SquareDistanceFunc<Z> SquareDistanceFuncType;
	public:
		SquareDistanceFunc() {
			this->_type = DistanceType::distanceSquare;
		}
		SquareDistanceFunc(const SquareDistanceFuncType &other) :DistanceFuncType(other) {
		}
		SquareDistanceFuncType & operator=(const SquareDistanceFuncType &other) {
			if (this != &other) {
				DistanceFuncType::operator=(other);
			}
			return (*this);
		}
		virtual ~SquareDistanceFunc() {}
	}; // class SquareDistanceFunc
	//////////////////////////////////////////
	...
	public:
		...
		//
		typedef DistanceFunc<Z> DistanceFuncType;
		typedef Khi2DistanceFunc<Z, ALLOCDOUBLE> Khi2DistanceFuncType;
	private:
	...
	public:
		...
		template <class CONTAINER>
		Khi2DistanceFunc(const size_t nRows, const size_t nCols, const CONTAINER &data)
		{
			assert(nRows > 1);
			assert(nCols > 1);
		...
		Khi2DistanceFunc(const Khi2DistanceFuncType &other) :
			DistanceFuncType(other), _colsum(other._colsum), _rowsum(other._rowsum) {}
	...
	public:
		const DoubleTypeVector & colWeights(void) const {
			return (this->_colsum);
		}
		template <class CONTAINER>
		void colWeights(const CONTAINER &ww) {
			const size_t n = ww.size();
			assert(n > 1);
			this->_colsum.resize(n);
			double total = 0;
			for (size_t i = 0; i < n; ++i) {
				double x = (double)ww[i];
				if (x < 0) {
					x = 0;
					this->_colsum[i] = x;
				}
			...
			if (total <= 0) {
				...
				total = 0;
				const size_t m = this->_rowsum.size();
				for (size_t i = 0; i < m; ++i) {
					double x = this->_rowsum[i];
					if (x < 0) {
						x = 0;
						this->_rowsum[i] = x;
					}
		...
		const DoubleTypeVector & rowWeights(void) const {
			return (this->_rowsum);
		}
		template <class CONTAINER>
		void rowWeights(const CONTAINER &ww) {
			const size_t n = ww.size();
			assert(n > 1);
			this->_rowsum.resize(n);
			double total = 0;
			for (size_t i = 0; i < n; ++i) {
				double x = (double)ww[i];
				if (x < 0) {
			...
			if (total <= 0) {
				...
				total = 0;
				const size_t m = this->_colsum.size();
				for (size_t i = 0; i < m; ++i) {
					double x = this->_colsum[i];
					if (x < 0) {
						x = 0;
						this->_colsum[i] = x;
					}
	...
	public:
		...
		DivergenceDistanceFuncType & operator=(const DivergenceDistanceFuncType &other) {
			if (this != &other) {
				DistanceFuncType::operator=(other);
			}
			return (*this);
	...
	protected:
	  ...
	  ////////////////////////////////////////
	template <typename Z = long, class ALLOCDOUBLE = std::allocator<double> > class WeightedDistanceFunc :
		public DistanceFunc<Z> {
	public:
		typedef std::vector<double, ALLOCDOUBLE> DoubleTypeVector;
		//
	...
	public:
		...
		WeightedDistanceFunc(const CONTAINER &ww) :
			_weights(ww.size()), _pfunc(&DistanceFuncType::DEFAULT_DISTANCE_FUNCTION) {
			const size_t n = ww.size();
			assert(n > 0);
			DoubleTypeVector &oAr = this->_weights;
			assert(oAr.size() == n);
		...
		template <class CONTAINER>
		WeightedDistanceFunc(const size_t nRows, const size_t nCols, const CONTAINER &oData) :
			_weights(nCols), _pfunc(&DistanceFuncType::DEFAULT_DISTANCE_FUNCTION) {
			assert(nRows > 0);
			assert(nCols > 0);
		...
		WeightedDistanceFunc(const WeightedDistanceFuncType &other) :
			DistanceFuncType(other), _weights(other._weights), _pfunc(other._pfunc) {}
	...
	protected:
		...
		double perform_compute(const CONTAINER1 & v1,
			...
			double res = 0;
			for (size_t i = 0; i < n; ++i) {
				double d = pfunc->perform_one_step((double)v1[i], (double)v2[i], i, irow1, irow2);
				res += d * oAr[i];
			}// i
			return (res);
		}// perform_compute
	...
	////////////////////////////////////////
	template <typename Z = long, class ALLOCDOUBLE = std::allocator<double> > class VarianceDistanceFunc :
		public WeightedDistanceFunc<Z, ALLOCDOUBLE> {
	public:
		typedef WeightedDistanceFunc<Z, ALLOCDOUBLE> WeightedDistanceFuncType;
		...
		//
		typedef VarianceDistanceFunc<Z, ALLOCDOUBLE> VarianceDistanceFuncType;
	public:
		VarianceDistanceFunc() {
			this->_type = DistanceType::distanceVariance;
			this->_valid = false;
		}
		template <class CONTAINER>
		...
		template <class CONTAINER>
		VarianceDistanceFunc(const size_t nRows, const size_t nCols, const CONTAINER &oData) :
	...
	//////////////////////////////////////////////////////////
	template <typename Z = long, class ALLOCDOUBLE = std::allocator<double> > class EigenDistanceFunc :
		public WeightedDistanceFunc<Z, ALLOCDOUBLE> {
	public:
		typedef WeightedDistanceFunc<Z, ALLOCDOUBLE> WeightedDistanceFuncType;
	...
	void info_distance(const CONTAINER1 &vv1, const CONTAINER2  &vv2, Z &result,
		...
		if (pFunc == nullptr) {
			result = DistanceFunc<Z>::DEFAULT_DISTANCE_FUNCTION(vv1, vv2, irow1, irow2);
		}
		else {
